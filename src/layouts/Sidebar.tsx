...
export default function Sidebar({
  ...
  return (
    <aside className={`${isOpen ? 'w-60' : 'w-0'} relative min-h-screen bg-amber-200 duration-500`}>
      <button
        onClick={toggleSidebar}
        className="absolute top-1/2 -left-7 -translate-y-1/2 cursor-pointer rounded-l-full bg-amber-200 p-2 hover:bg-amber-300"
      >
        <span className="w-fit font-bold text-gray-900">{isOpen ? '>' : '<'}</span>
      </button>
      {isOpen && (
        <form>
            ...
            return (
              <div key={name} className="m-4">
                <>
                  ...
                  >
                    {[{ label: 'Select...', value: '' }, ...options].map((option) => (
          ...
          <StarRating value={sidebarForm.rating} onChangeRating={(rating) => onRatingChange('rating', rating)} />
        </form>
      )}
    </aside>
  );
